...
class HumanoidStandupEnv(MujocoEnv, utils.EzPickle):
    ...
    **Note:**
    ...
    | `reward_type`                                | **str**   | `"dense"`             | The type of reward to use. `"dense"` gives the dense reward, while `"sparse"` gives a sparse reward of 0 or -1 depending on whether the humanoid is standing or not.                                        |
    | `height_th`                                  | **float** | `0.6`                 | The threshold height for the sparse reward. If the humanoid's torso is above this height, the sparse reward is 0, otherwise, it is -1.                                                                        |
    ...
    * v0: Initial versions release.
    ...
    def _get_rew(self, pos_after: float, action):
        ...
        dense_reward = uph_cost - quad_ctrl_cost - quad_impact_cost + 1
        if self._reward_type == "sparse":
            reward = float(pos_after >= self._height_th) - 1.0
        else:
            reward = dense_reward
