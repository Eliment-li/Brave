...
if __name__ == "__main__":
    ...
    parser.add_argument('--gym-id', type=str, default="Acrobot-v1",
                        help='the id of the gym environment')
    parser.add_argument('--learning-rate', type=float, default=7e-4,
                        help='the learning rate of the optimizer')
    parser.add_argument('--seed', type=int, default=1,
                        help='seed of the experiment')
    parser.add_argument('--total-timesteps', type=int, default=20*1000*1000,
    ...
    parser.add_argument('--prod-mode', type=lambda x: bool(strtobool(x)), default=True, nargs='?', const=True,
                        help='run the script in production mode and use SwanLab to log outputs')
    parser.add_argument('--capture-video', type=lambda x: bool(strtobool(x)), default=False, nargs='?', const=True,
                        help='whether to capture videos of the agent performances (check out `results/videos` folder)')
    parser.add_argument('--swanlab-project', type=str, default="Brave",
                        help='the SwanLab project name')
    parser.add_argument('--swanlab-workspace', type=str, default=None,
                        help='the SwanLab workspace/team')
...
# Remove SummaryWriter and use SwanLab instead
if args.prod_mode:
    swanlab.init(
        project="Brave",
...
if args.capture_video:
    # Save videos under results/videos similar to dqn2.py
    env = RecordVideo(env, str(Path(get_root_path()) / "results" / "videos"))
...
class ReplayBuffer():
    ...
    def sample(self, n):
        ...
        return np.array(s_lst), np.array(a_lst), \
            np.array(a_lst), np.array(s_prime_lst), \
...
for global_step in range(args.total_timesteps):
    ...
    if global_step > args.learning_starts and global_step % args.train_frequency == 0:
        ...
        if global_step % 100 == 0:
            # Log to SwanLab instead of SummaryWriter
            if args.prod_mode:
                try:
...
env.close()
# writer.close()  # removed SummaryWriter usage
if args.prod_mode:
    swanlab.finish()
